...
// //         const resolvedMessages = await Promise.all(
// //           emails.map(async email => {
// //             let resolvedMessage = await resolveTemplate(id, content, email, webinar_id);
// //             console.log(`Resolved message for ${email}: ${resolvedMessage.substring(0, 100)}...`);

// //             // Replace relative image URLs with absolute URLs in the resolved message
// //             const baseUrl = process.env.BASE_URL || 'http://yourserver.com';
// //             resolvedMessage = resolvedMessage.replace(
// //               /src="\/uploads\//g,
// //               `src="${baseUrl}/uploads/`
// //             );

// //             // Add inline styles to images for better email client compatibility
// //             resolvedMessage = resolvedMessage.replace(
// //               /<img src="[^"]*"/g,
// //               match => `${match} style="max-width: 100%; height: auto; border-radius: 8px; margin: 10px 0;"`
// //             );
...
// //             ? `${baseUrl}${image}`
// //             : 'https://yourwebsite.com/path-to-default-thumbnail.jpg';

// //           return {
// //             email,
// //             htmlContent: `
// //               <div style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px; border-radius: 10px; text-align: center; max-width: 600px; margin: 0 auto;">
// //                 <h2 style="color: #007bff; margin-bottom: 20px;">📚 <b>LMS Notification</b></h2>
// //                 ${thumbnailImage ? `
// //                   <div style="margin-bottom: 20px;">
// //                     <img src="${thumbnailImage}" alt="LMS Notification" style="width: 100%; max-width: 400px; height: auto; border-radius: 8px; display: block; margin: 0 auto;">
// //                   </div>
// //                 ` : ''}
...
// //                 <p style="margin-top: 20px; font-size: 14px; color: #555;">
// //                   If you no longer wish to receive these emails,  
// //                   <a href="${recipientData.find(r => r.email === email).unsubscribe_link}" style="color: #ff0000; text-decoration: none; font-weight: bold;">unsubscribe</a>
// //                 </p>
// //               </div>
// //             `,
// //           };
...
// import { sendEmail } from "../utils/emailSender.js";
// import { resolveTemplate } from "./templateHelper.js";
// import axios from 'axios'; // MODIFIED: Added axios to fetch images
...
//         // MODIFIED: Fetch the image if it exists
//         const baseUrl = process.env.BASE_URL || 'http://yourserver.com';
//         let attachments = [];
//         if (image) {
...
//             attachments.push({
//               filename: image.split('/').pop() || 'thumbnail.jpg', // Extract filename or use default
//               content: Buffer.from(response.data),
//             });
//             console.log(`Attaching image to email: ${imageUrl}`);
//           } catch (err) {
//             console.error(`Failed to fetch image from ${imageUrl}:`, err.message);
//           }
//         }
...
//         for (const { email, htmlContent } of htmlContents) {
//           try {
//             // MODIFIED: Pass attachments to sendEmail
//             const initialResponse = await sendEmail([email], "LMS Notification", htmlContent, attachments);
//             console.log(`Initial Response for email to ${email}:`, initialResponse);
//             console.log(`✅ Email sent to ${email} with Message ID: ${initialResponse.messageId}`);
...
export const checkAndSendEmails = async (notificationId, groups, webinarId, selectedTemplateId) => {
  ...
  try {
    ...
    console.log(`Found ${notifications.length} email notifications to send...`);
    for (const notification of notifications) {
      const { id, content, image, groups: notificationGroups, webinar_id, user_id } = notification;
      ...
      // Fetch the reply_to_email from email_settings for the user
      let replyToEmail = null;
      if (user_id) {
        const emailSettingsResult = await client.query(
        ...
        if (emailSettingsResult.rows.length > 0) {
          replyToEmail = emailSettingsResult.rows[0].reply_to_email;
          console.log(`Using reply_to_email: ${replyToEmail} for user_id: ${user_id}`);
        } else {
          console.log(`No email settings found for user_id: ${user_id}. Using default sender email.`);
        }
      } else {
        console.log(`No user_id found for notification ID ${id}. Using default sender email.`);
      }
      ...
      if (emails.length > 0) {
        ...
        if (image) {
          ...
          try {
            const response = await axios.get(imageUrl, { responseType: 'arraybuffer' });
            attachments.push({
              filename: image.split('/').pop() || 'thumbnail.jpg',
              content: Buffer.from(response.data),
            });
            console.log(`Attaching image to email: ${imageUrl}`);
          } catch (err) {
        ...
        for (const { email, htmlContent } of htmlContents) {
          try {
            const initialResponse = await sendEmail(
              [email],
              "LMS Notification",
              htmlContent,
              attachments,
